...
// BuildErrorContext создаёт контекст ошибки с похожими элементами
func BuildErrorContext(ctx context.Context, p PageProvider, failedSelector string, err error) *domain.ErrorContext {
	similar := findSimilarElements(p)
	return &domain.ErrorContext{
		FailedSelector:  failedSelector,
		SimilarElements: similar,
		Suggestion:      generateSuggestion(failedSelector, err, similar),
	}
}

func findSimilarElements(p PageProvider) []string {
	var results []string
	page := p.GetPage()
	selectors := []string{"button", "a[href]", "input", "[role='button']", "[data-testid]"}

	for _, sel := range selectors {
		elements, err := page.Timeout(time.Second).Elements(sel)
		if err != nil {
			continue
		}
...
func generateSuggestion(selector string, err error, similar []string) string {
	errStr := err.Error()
	switch {
	case strings.Contains(errStr, "not found") || strings.Contains(errStr, "timeout"):
		if strings.Contains(selector, "modal") {
			return "Модальное окно закрыто. Открой его снова."
		}
		if len(similar) > 0 {
			return "Элемент не найден. Используй альтернативный селектор."
		}
