...
// FindElementsLive ищет кликабельные элементы в реальном DOM
func FindElementsLive(ctx context.Context, p PageProvider, query string) (string, error) {
	page := p.GetPage()
	result, err := page.Timeout(5 * time.Second).Eval(findElementsJS)
	if err != nil {
	...
	logger.Info(ctx, "🔍 FindElementsLive", zap.Int("found", len(elements)), zap.String("query", query))
