...
func (m confirmModel) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	if key, ok := msg.(tea.KeyMsg); ok {
		switch key.String() {
		case "up", "k", "left", "h": m.selected = 0
		case "down", "j", "right", "l": m.selected = 1
		case "enter", " ": m.confirmed, m.quitting = m.selected == 0, true; return m, tea.Quit
		case "y", "Y": m.confirmed, m.quitting = true, true; return m, tea.Quit
		case "n", "N", "q", "esc", "ctrl+c": m.cancelled, m.quitting = true, true; return m, tea.Quit
		}
	}
	return m, nil
}
...
func (m confirmModel) View() string {
	if m.quitting {
		if m.confirmed { return selectedStyle.Render("✓ Выполняю...\n") }
...
func (m confirmModel) details() string {
	...
	if m.action.Selector != "" {
		sel := m.action.Selector; if len(sel) > 50 { sel = sel[:50] + "..." }
		d.WriteString(labelStyle.Render("Селектор: ") + valueStyle.Render(sel) + "\n")
	}
	if m.action.Value != "" {
		val := m.action.Value; if len(val) > 50 { val = val[:50] + "..." }
...
func (m confirmModel) options() string {
	...
	if m.selected == 0 { yes = "▶ Выполнить" } else { no = "▶ Отменить" }
	return unselectedStyle.Render(yes) + "\n" + selectedStyle.Render(no) + "\n"
}

func ConfirmAction(action domain.Action, risk Risk) (bool, error) {
	m := confirmModel{action: action, risk: risk, selected: 1}
	p := tea.NewProgram(m)
	final, err := p.Run()
	if err != nil { return false, fmt.Errorf("dialog failed: %w", err) }
