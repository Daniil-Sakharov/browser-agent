...
func (a *Agent) executeStep(ctx context.Context) (bool, error) {
	...
	if err != nil { return false, fmt.Errorf("page: %w", err) }

	msg := ""; if a.stepCount == 1 { msg = a.currentTask.Description }
	...
	if err != nil { return false, fmt.Errorf("AI: %w", err) }
	...
	if err != nil { a.ai.AddToolResult(d.ToolUseID, "Error: "+err.Error(), true); return false, err }
...
func (a *Agent) emitToolProgress(d *domain.Decision) {
	p := map[string]string{}
	if d.Action.Selector != "" { p["selector"] = d.Action.Selector }
	if d.Action.URL != "" { p["url"] = d.Action.URL }
...
func (a *Agent) handleActionResult(ctx context.Context, d *domain.Decision, r *domain.ActionResult) {
	if !r.Success { a.handleFailedAction(ctx, d, r) } else { a.consecutiveFailures, a.lastFailedAction = 0, "" }
	...
	switch {
	case !r.Success && r.ErrorContext != nil: a.ai.AddToolResult(d.ToolUseID, formatErrorContextMessage(r), true)
	case r.ScreenshotB64 != "": a.ai.AddToolResultWithImage(d.ToolUseID, r.Message, r.ScreenshotB64, !r.Success)
	default: a.ai.AddToolResult(d.ToolUseID, r.Message, !r.Success)
	}
}
