...
func (d *DIContainer) BrowserController(ctx context.Context) *browser.Controller {
	if d.browserController == nil {
		...
		if err != nil { panic(fmt.Sprintf("browser: %s", err)) }
...
func (d *DIContainer) AIClient(ctx context.Context) *ai.Client {
	if d.aiClient == nil {
		...
		if err != nil { panic(fmt.Sprintf("ai: %s", err)) }
...
func (d *DIContainer) SecurityChecker(ctx context.Context) *security.Checker {
	if d.securityChecker == nil {
		callback := func(ctx context.Context, a domain.Action, r security.Risk) (bool, error) {
			fmt.Println()
			return security.ConfirmAction(a, r)
		}
		checker, err := security.New(ctx, config.AppConfig().Security, callback)
		if err != nil { panic(fmt.Sprintf("security: %s", err)) }
...
func (d *DIContainer) DOMSubAgent(ctx context.Context) *ai.DOMSubAgent {
	if d.domSubAgent == nil {
		cfg := config.AppConfig().Anthropic
		d.domSubAgent = ai.NewDOMSubAgent(cfg.APIKey(), cfg.BaseURL(), cfg.Model(), 2048)
	}
...
func (d *DIContainer) Agent(ctx context.Context) *agent.Agent {
	if d.agent == nil {
		...
		if err != nil { panic(fmt.Sprintf("agent: %s", err)) }
